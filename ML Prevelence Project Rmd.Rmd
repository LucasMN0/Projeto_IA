...
```{r include = T}
safe_plot <- function(pm, fm, title, yesno=1) {
  if (nrow(pm$frame) > 1) {
    rpart.plot(pm, main=title, box.palette="RdYlGn", yesno=yesno)
  } else {
    rpart.plot(fm, main=paste(title, "(árvore completa — poda resultou em nó raiz)"),
               box.palette="RdYlGn", yesno=yesno)
  }
}
safe_plot(pmodel_cal, model_cal, "Regression Tree of AT Prevelence in Calgary")
safe_plot(pmodel_pee, model_pee, "Regression Tree of AT Prevelence in Peel Region")
safe_plot(pmodel_tor, model_tor, "Regression Tree of AT Prevelence in Toronto", yesno=2)
safe_plot(pmodel_mon, model_mon, "Regression Tree of AT Prevelence in Montreal", yesno=2)
safe_plot(pmodel_lav, model_lav, "Regression Tree of AT Prevelence in Laval")
safe_plot(pmodel_sur, model_sur, "Regression Tree of AT Prevelence in Surrey")
safe_plot(pmodel_van, model_van, "Regression Tree of AT Prevelence in Vancouver", yesno=2)
```
...
safe_vip <- function(pm, fm, title = "") {
  m <- if (!is.null(pm$variable.importance) && nrow(pm$frame) > 1) pm else fm
  p <- vip::vip(m)
  if (nchar(title) > 0) p <- p + ggplot2::ggtitle(title)
  p
}
var_importance_pcal = safe_vip(pmodel_cal, model_cal, "Calgary (pruned)")
var_importance_ppee = safe_vip(pmodel_pee, model_pee, "Peel Region (pruned)")
var_importance_ptor = safe_vip(pmodel_tor, model_tor, "Toronto (pruned)")
var_importance_pmon = safe_vip(pmodel_mon, model_mon, "Montreal (pruned)")
var_importance_plav = safe_vip(pmodel_lav, model_lav, "Laval (pruned)")
var_importance_psur = safe_vip(pmodel_sur, model_sur, "Surrey (pruned)")
var_importance_pvan = safe_vip(pmodel_van, model_van, "Vancouver (pruned)")
...
ggsave(impplot, filename = "output/AT_Importance_Score_Dot_Plot.png")
impplot
```
